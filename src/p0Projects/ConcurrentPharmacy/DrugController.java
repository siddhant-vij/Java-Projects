...
public class DrugController {
  ...
  public static synchronized void buy(Drug drug, int count) {
    ...
    if (currentCount < count) {
      System.out
          .println(String.format("%s wants %d tablets of %s. In stock: %d", name, count, drug.getName(), currentCount));
    } else {
      allDrugs.put(drug, (currentCount - count));
      System.out.println(String.format("%s bought %d tablets of %s. Remaining: %d", name, count, drug.getName(),
          (currentCount - count)));
    }
  }
